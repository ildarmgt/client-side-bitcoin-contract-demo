...
const WHERE_TO_SEND = '3JQtLGuiwPgYp5AW44b5so69gzr4oDVnSh'; // my wallet
...
const myKeyPair = bitcoin.ECPair.fromWIF(MY_PRIVATE_KEY, network)
const mySignature = bitcoin.script.signature.encode(
  myKeyPair.sign(hashForSignature), hashType
);

// submit values and original script
const inputScriptMySpending = bitcoin.payments.p2sh({
  redeem: {
...
tx.setInputScript(0, inputScriptMySpending);
